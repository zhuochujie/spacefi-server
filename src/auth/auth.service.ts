...
import { buildLoginMessage, buildRegisterMessage } from 'src/web3/account-message.util';
...
export class AuthService {
    ...
    async login(address: string, signature: string): Promise<{ access_token: string }> {
        ...
        const res = await this.web3Service.accountVerify(address, message, signature);
        if (!res) {
