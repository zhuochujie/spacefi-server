...
import { AccountBalanceLog, AccountBalanceLogToken, AccountBalanceLogType } from './entities/account-balance-log.entity';
...
export class AccountService {
    ...
    private static readonly DIVIDEND_FALLBACK_ACCOUNT_ADDRESS = '0x0000000000000000000000000000000000000000';
    ...
    async createAccount(address: string, refCode: string) {
        return await this.dataSource.transaction(async (manager) => {
            ...
            try {
                nodeLevel = await this.web3Service.getNodeLevel(address as '0x${string}');
            } catch (error) {
                nodeLevel = 0;
            }
            const currentTimestamp = Math.floor(Date.now() / 1000);
    ...
    async syncNodeLevel(address: string) {
        const nodeLevel = await this.web3Service.getNodeLevel(address as '0x${string}');
        const accountRepository = this.dataSource.getRepository(Account);
    ...
    async claimFeeExempt(accountId: number) {
        ...
        const feeExemptMinNodeLevel = await this.configService.getFeeExemptMinNodeLevel();
    ...
    async getCommissionLevel(accountId: number): Promise<{ commissionLevel: number }> {
    ...
    async getTeam(accountId: number) {
        const directList = await this.dataSource.query<{
            id: number;
            address: string;
            refCode: string;
            vipLevel: number;
    ...
    async generateWithdrawSignature(accountId: number, amount: string) {
        return await this.dataSource.transaction(async manager => {
            ...
            const withdrawSignatureRepository = manager.getRepository(AccountWithdrawSignature);
            const currentTimestamp = Math.floor(Date.now() / 1000);
            ...
            const { vipFeeBp, nodeFeeBp } = await this.configService.getWithdrawFeeBps();
    ...
    async generateWithdrawUsdtSignature(accountId: number, amount: string) {
        return await this.dataSource.transaction(async manager => {
            ...
            account.usdtBalance = (BigInt(account.usdtBalance) - withdrawAmount).toString();
            await accountRepository.save(account);
            await balanceLogRepository.save(
                balanceLogRepository.create({
                    accountId: account.id,
    ...
    async checkWithdrawSignature() {
        const currentTimestamp = Math.floor(Date.now() / 1000);
        const gracePeriod = 2 * 60;
        const withdrawSignatureRepository = this.dataSource.getRepository(AccountWithdrawSignature);
    ...
    private async processWithdrawSignature(
        ...
        await this.dataSource.transaction(async manager => {
            ...
            const balanceField = lockedSignature.token === AccountBalanceLogToken.Usdt
                ? 'usdtBalance'
                : 'balance';
            const balanceBefore = account[balanceField];
            account[balanceField] = (BigInt(account[balanceField]) + refundAmount).toString();
            await accountRepository.save(account);
    ...
    private async getWithdrawSignaturesUsed(signatures: AccountWithdrawSignature[]) {
        const usedResults = new Map<number, boolean>();
        const spaceSignatures = signatures.filter(signature => signature.token === AccountBalanceLogToken.Space);
        const usdtSignatures = signatures.filter(signature => signature.token === AccountBalanceLogToken.Usdt);
        ...
        return signatures.map(signature => usedResults.get(signature.id) ?? false);
    }

    // 分红
    @Cron(CronExpression.EVERY_WEEK, { timeZone: 'Asia/Shanghai' })
    async dividend() {
        const { vipFeeVaultSpaceBalance, nodeFeeVaultSpaceBalance, nodeFeeVaultUsdtBalance } = await this.web3Service.dividend();
        const currentTimestamp = Math.floor(Date.now() / 1000);
    ...
    private async distributeDividendToRecipients(
        ...
        const accountRepository = manager.getRepository(Account);
        const balanceLogRepository = manager.getRepository(AccountBalanceLog);
        const targetIds = recipientIds.length > 0
            ? recipientIds
        ...
        for (const accountId of targetIds) {
            ...
            const balanceBefore = account[balanceField];
            account[balanceField] = (BigInt(account[balanceField]) + averageAmount).toString();
            await accountRepository.save(account);
            await balanceLogRepository.save(
                balanceLogRepository.create({
                    accountId: account.id,
    ...
    private async getDividendFallbackAccountId(accountRepository: Repository<Account>) {
        const account = await accountRepository.findOne({
            where: {
